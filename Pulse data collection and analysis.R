...
# Read in the data
pulse <- read_excel("UNGA_VIDEO.xlsx") #UNGA_VIDEOS: analytics of "Pulse" videos
...
view_plot <- ggplot(plot_data_view, aes(x = group, y = value_adj, fill = group)) +
  ...
  geom_line(aes(group = id), color = "gray60", alpha = 0.5, linewidth = 0.6) +
  ...
  labs(x = "Release Strategy", y = "View Count") +
  theme(legend.position = "none", plot.title = element_text(face = "bold", hjust = 0.5)) +
  annotate(
    "text",
    x = 1,
    y = max(plot_data_view$value_adj, na.rm = TRUE) * 1.2,
    label = bquote(
      Delta == .(estimate) ~ "(" * 95*"% CI:" * .(ci_lower) * "-" * .(ci_upper) * ")" * 
        "   " *    # extra space
        italic(P) == .(p_val)
    ),
    size = 5
...
# Plot like count
like_plot <- ggplot(plot_data_like, aes(x = group, y = value, fill = group)) +
  geom_boxplot(outlier.shape = 16, outlier.size = 2, alpha = 0.7) +
  geom_line(aes(group = id), color = "gray60", alpha = 0.5, linewidth = 0.6) +
  ...
  annotate(
    "text",
    x = 1,
    y = max(plot_data_like$value_adj, na.rm = TRUE) * 1.2,
    label = bquote(
      Delta == .(estimate) ~ "(" * 95*"% CI:" * .(ci_lower) * "-" * .(ci_upper) * ")" * 
        "   " *    # extra space
        italic(P) == .(p_val)
...
comment_plot <- ggplot(plot_data_comment, aes(x = group, y = value, fill = group)) +
  ...
  annotate(
    "text",
    x = 1,
    y = max(plot_data_comment$value_adj, na.rm = TRUE) * 1.1,
    label = bquote(
      Delta == .(estimate) ~ "(" * 95*"% CI:" * .(ci_lower) * "-" * .(ci_upper) * ")" * 
        "   " *    # extra space
        italic(P) == .(p_val)
...
unga # file containing the videos released during "Pulse"
...
process_creator <- function(file_path) {
  ...
  # Combine coordinated video with baseline averages to work out ratios
  result <- coord %>%
    select(creator, views, likes, comments) %>%
    bind_cols(baseline) %>%
    mutate(
      view_ratio = views / avg_views,
      like_ratio = likes / avg_likes,
      comment_ratio = comments / avg_comments
    )
...
#Dataframe of each creator and the ratio of engagegement between "Pulse" video and the average creator's videos
plot_data <- comparison_data %>%
  select(creator, view_ratio, like_ratio, comment_ratio) %>%
  pivot_longer(cols = -creator, names_to = "metric", values_to = "ratio")
plot_data
names(plot_data)

plot_data$metric <- factor(plot_data$metric, levels = c("view_ratio", "like_ratio", "comment_ratio"))

plot_data
library(dplyr)
# Wilcoxon test of "Pulse" video against baseline average of the creator
#with estimates, confidence intervals, and P values

summary_results <- plot_data %>%
  group_by(metric) %>%
  summarise(
    estimate = median(ratio - 1, na.rm = TRUE),
    conf_low = quantile(ratio - 1, 0.025, na.rm = TRUE),
    conf_high = quantile(ratio - 1, 0.975, na.rm = TRUE),
    p_value = wilcox.test(ratio - 1, mu = 0, exact = FALSE)$p.value
  )
summary_results
...
plot_data_filtered <- plot_data %>% filter(ratio > 0)
...
ggplot(plot_data_filtered, aes(x = metric, y = ratio)) +
  ...
  scale_y_continuous(
    ...
    limits = c(min(plot_data_filtered$ratio, na.rm = TRUE) * 0.9, 10^2)
  ) +
  theme_minimal(base_size = 14) +
  labs(
    title = "",
...
process_creator <- function(file_path) {
  ...
  # Get the format of the coordinated video
...
# Process all 18 creator files
comparison_data2 <- map_dfr(files, process_creator)
comparison_data2
...
plot_data2$metric <- factor(plot_data2$metric, levels = c("view_ratio", "like_ratio", "comment_ratio"))
